...
export default function Navigation({ theme, setTheme, activeNav, setActiveNav }) {
    ...
    const nav_link_color = (ActiveTab) => {
        return `cursor-pointer nav__link Underline ${!theme ? (activeNav == ActiveTab ? 'text-dark_text_inverse' : 'text-light_text_secondary') : (activeNav == ActiveTab ? 'text-light_text_inverse' : 'text-dark_text_secondary')} ${!theme ? 'after:bg-light_text_secondary hover:text-dark_text_inverse' : 'after:bg-dark_text_secondary hover:text-light_text_inverse'}`
    }
        ...
        <header className='header' id="header">
            ...
            <nav className={`nav container ${nav_nav_menu_sm_color}`}>
                ...
                <div className={`nav__menu ${open && 'show-menu'} ${nav_nav_menu_sm_color}`} id="nav-menu">
                    <ul className="nav__list myGrid gap-6">
                        <li className="nav__item">
                            <Link href="#About" className={`${nav_link_color("About")}`} onClick={() => { setOpen(false); setActiveNav("About") }}>
                                <GoPerson className="nav__icon" />About
                            </Link>
                        </li>
                        <li className="nav__item">
                            <Link href="#Projects" className={`${nav_link_color("Projects")}`} onClick={() => { setOpen(false); setActiveNav("Projects") }}>
                                <GoProject className="nav__icon" />Projects
                            </Link>
                        </li>
                        <li className="nav__item">
                            <Link href='#skills' className={`${nav_link_color("Skills")}`} onClick={() => { setOpen(false); setActiveNav("Skills") }}>
                                <MdOutlineWorkOutline className="nav__icon" />Skills
                            </Link>
                        </li>
                        {/* <li className="nav__item">
                            <Link href="#Achievements" className={`${nav_link_color("Achievements")}`} onClick={() => { setOpen(false); setActiveNav("Achievements") }}>
                                <GiAchievement className="nav__icon" />Achievements
                            </Link>
                        </li> */}
                        <li className="nav__item">
                            <Link href="#Contact" className={`${nav_link_color("Contact")}`} onClick={() => { setOpen(false); setActiveNav("Contact") }}>
